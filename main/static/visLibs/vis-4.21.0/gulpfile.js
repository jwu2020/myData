...
var INDIVIDUAL_CSS_BUNDLES = [
  {entry: ['./bin/shared/**/*.css', './bin/timeline/**/*.css'], filename: 'vis-timeline-graph2d.min.css'},
  {entry: ['./bin/shared/**/*.css', './bin/network/**/*.css'], filename: 'vis-network.min.css'}
];
...
gulp.task('copy', ['clean'], function () {
    var network = gulp.src('./bin/network/img/**/*')
...
gulp.task('watch', watchTasks, function () {
  gulp.watch(['index.js', 'bin/**/*'], watchTasks);
});
...
gulp.task('lint', function () {
  return gulp.src(['bin/**/*.js', '!node_modules/**'])
    .pipe(eslint())
    .pipe(eslint.format())
